...
#    mdj, rdj, xdj, ydj = diffPlanet(Jupiter, Earth)    
    ...
    for planet in planet_list: 
        ...
        #x_old = planet.x
        planet.x_acc_old = np.sum(planet.forcex)/planet.m
        planet.y_acc_old = np.sum(planet.forcey)/planet.m
        ...
        planet.y = planet.y + h*planet.vy + h**2/2*(np.sum(planet.forcey)/planet.m)
...
#        planet.vy = planet.vy - h*4*np.pi**2*(y_old/planet.r**3 + np.sum(planet.forcey))
        x_acceleration = np.sum(planet.forcex)/planet.m
        y_acceleration = np.sum(planet.forcey)/planet.m
        planet.vx = planet.vx + h/2*((planet.x_acc_old) + (x_acceleration))
        planet.vy = planet.vy + h/2*((planet.y_acc_old) + (y_acceleration))
